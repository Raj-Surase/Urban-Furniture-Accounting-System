...
export const AnalyticAccountsPage: React.FC = () => {
  ...
  const [type, setType] = useState<BudgetLineType>(BudgetLineType.EXPENSE);
  ...
  const handleOpenForm = async (account?: AnalyticAccount) => {
    ...
    if (account) {
      ...
      setName('');
      setCode('');
      setType(BudgetLineType.EXPENSE);
      setDescription('');
      setIsActive(true);
      setRelatedBudgets([]);
    }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 bg-[#121216] border border-white/[0.08] rounded-xl text-xs text-white focus:outline-none focus:border-[#7042f4]"
              >
                <option value={BudgetLineType.EXPENSE}>Expense (Procurement & Vendor Bills)</option>
                <option value={BudgetLineType.INCOME}>Income (Sales & Customer Invoices)</option>
              </select>
              <span className="text-[10px] text-[#606070] mt-1 block">
