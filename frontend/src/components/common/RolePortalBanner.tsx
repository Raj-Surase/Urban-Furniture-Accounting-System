...
export const RolePortalBanner: React.FC<RolePortalBannerProps> = ({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-xs font-bold text-white tracking-tight">{roleTitle}</span>
      ...
      <div className="flex items-center gap-2 text-[11px] text-neutral-300 sm:text-right shrink-0">
        <div className="p-1.5 rounded-lg bg-white/[0.04] border border-white/[0.06] flex items-center gap-1.5">
          <UserCheck className="w-3.5 h-3.5 text-emerald-400" />
          <span className="font-semibold text-white">{partnerName}</span>
          <span className="text-[9.5px] uppercase font-bold tracking-wider px-1.5 py-0.2 rounded bg-white/[0.08] text-neutral-400">
            {user?.role}
          </span>
        </div>
      </div>
