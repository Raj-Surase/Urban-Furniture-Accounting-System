...
export const MasterViewLayout: React.FC<MasterViewLayoutProps> = ({
  ...
  return (
    <div className="space-y-6 pb-12">
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#18181f]/80 backdrop-blur-xl border border-white/[0.08] p-5 rounded-2xl shadow-obsidian-card">
        <div>
          ...
          {subtitle && (
            <p className="text-xs sm:text-sm text-[#8a8a9a] mt-1 ml-0.5">
              {subtitle}
            </p>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {viewMode !== 'form' && onSearchChange && (
            <div className="relative min-w-[220px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-[#121216] border border-white/[0.08] rounded-xl text-xs text-white placeholder-[#606070] focus:outline-none focus:border-[#7042f4] transition-all"
              />
            </div>
          )}
          ...
          {viewMode !== 'form' && onViewModeChange && (
            <div className="flex items-center bg-[#121216] border border-white/[0.08] p-1 rounded-xl">
              <button
